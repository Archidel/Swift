...
/**Class for get tariff list.
 * This class has implementation {@link Command}.
 * @author Albert Zarankovich
 */
public class GetTariffListUser implements Command {
	private static final Logger LOGGER = Logger.getLogger(GetTariffListUser.class);
	...
	public void executeCommand(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		try {
			...
			int numberEntries = tariffService.getAmountEntriesListTariffActiveStatus(tariffType);
			...
			List<Tariff> list = tariffService.getListTariffActiveStatus(numberPage, tariffType);
			request.setAttribute(AttributeName.LIST, list);
			request.getRequestDispatcher(PageName.TARIFF_LIST_USER_PAGE).forward(request, response);
